...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the page loads with Save
    expect(getByText(appointment, "Save")).toBeInTheDocument();

    //5. edit the exiting information
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
    ...
    // // 7. checks the "Saving" keyword as we know it goes from CREATE > SAVING (STATUS) > SHOW. mean's it's working correctly after a click
    expect(getByText(appointment, "Saving")).toBeInTheDocument();

    // //8.  Wait until the text "New Name" is displayed.
